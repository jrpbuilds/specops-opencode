...
- relevant tests, typecheck, lint, build, focused checks, or manual checks pass where practical
- no material regressions or obvious requirement gaps remain

Run `openspec validate <change>` as an additional structural check.

If a required task or verification cannot actually be performed in this environment, do not issue PASS. Leave the task unchecked and return FAIL with a concrete "pending required verification" finding naming the task and unavailable capability. Do not fake, infer, or assume completion.
