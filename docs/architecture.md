...
The failure mode this guards against is hard-coding so much orchestration into TypeScript that the Orchestrator becomes a puppet executing a workflow engine, unable to weigh evidence, trade-offs, or risk. Deterministic mechanics exist to remove prose, ambiguity, and repeated explanation from prompts — never to remove judgement from the agents.
...
Every lifecycle rule has exactly one deterministic home. Planning completion lives in `src/orchestrator/planning-completion.ts`, and the status projection, the planning scheduler, and the Todo projection all consume that same verdict — none of them re-derive the answer from raw state. If a surface appears to answer a lifecycle question independently, it is drift: wire it to the canonical helper instead. This keeps the invariant simple — when `specops_status` reports an action as legal, the tool boundaries that enforce the same rule agree.
...
Review breadth and risk weighting stay model-owned unless a future policy explicitly makes a specific rule deterministic. For example, critical-path priority when implementation lanes exceed concurrency is orchestrator guidance — a judgement expressed in the prompt — not a TypeScript scheduler decision.
...
- Recommended: whether to start implementation now, or which eligible work to dispatch first, is the orchestrator's judgement.
...
> If TypeScript determines which valid engineering choice the Orchestrator should make, rather than whether a choice is valid, the behaviour requires explicit architectural justification.
...
If you find yourself writing TypeScript that picks _which_ valid engineering choice the Orchestrator should make, stop: either move the choice back to the model, or write down the explicit architectural justification for making it deterministic.
