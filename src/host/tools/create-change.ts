...
import { CREATE_CHANGE_INPUT, optionalStringField, stringField, type ToolDraft } from "./shared.js";

export function addCreateChangeTool(tools: ToolDraft, ctx: Plugin.Context): void {
    tools.add({
        name: "specops_create_change",
        ...
        execute: async (input, context) => {
            ...
            return {
                content: await createChange(change, goal, {
                    createChange: (name, requestedGoal) => createOpenSpecChange(name, directory, requestedGoal),
                }),
            };
        },
    });
